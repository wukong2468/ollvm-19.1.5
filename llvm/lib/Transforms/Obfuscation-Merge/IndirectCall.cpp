...
struct IndirectCall : public FunctionPass {
  ...
  /// 查找 call function 指令，也就是函数调用指令，这个指令之后需要加密然后通过加密后的指令地址间接调用到目标函数
  ...
  GlobalVariable * getIndirectCallees2(Function &F, ConstantInt *AddKey, ConstantInt *XorKey) {
    ...
    for (auto Callee:Callees) {
      Constant *CE = ConstantExpr::getBitCast(
        Callee, PointerType::getUnqual(F.getContext()));
      CE = ConstantExpr::getGetElementPtr(Type::getInt8Ty(F.getContext()), CE, ConstantExpr::getXor(AddKey, ConstantExpr::get(Instruction::Mul, XorKey, ConstantInt::get(intType, CalleeNumbering[Callee], false))));
      Elements.push_back(CE);
    }
