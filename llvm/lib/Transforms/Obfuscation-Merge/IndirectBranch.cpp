...
struct IndirectBranch : public FunctionPass {
  ...
  GlobalVariable *getIndirectTargets2(Function &F, ConstantInt *AddKey, ConstantInt *XorKey) {
    ...
    for (auto BB:BBTargets) {
      Constant *CE = ConstantExpr::getBitCast(BlockAddress::get(BB), PointerType::getUnqual(F.getContext()));
      CE = ConstantExpr::getGetElementPtr(Type::getInt8Ty(F.getContext()), CE, ConstantExpr::getXor(AddKey, ConstantExpr::get(Instruction::Mul, XorKey, ConstantInt::get(intType, BBNumbering[BB], false))));
      Elements.push_back(CE);
    }
  ...
  /// 指令间接跳转，只有这个函数里存在至少一个if条件分支才能实现间接跳转
  bool runOnFunction(Function &Fn) override {
    ...
    // hasLinkOnceLinkage 跳过不能混淆的函数类型（如声明、内联、虚函数等）
